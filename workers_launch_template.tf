...
resource "aws_autoscaling_group" "workers_launch_template" {
  ...
  wait_for_capacity_timeout = "${lookup(var.worker_groups_launch_template[count.index], "wait_for_capacity_timeout", local.workers_group_launch_template_defaults["wait_for_capacity_timeout"])}"
